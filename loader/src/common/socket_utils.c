...
ssize_t write_string(int fd, const char *str) {
  ...
  ssize_t write_bytes = write_n(fd, &str_len, sizeof(size_t));
  ...
  write_bytes = write_n(fd, str, str_len);
...
char *read_string(int fd) {
  size_t str_len = 0;
  ssize_t read_bytes = read_n(fd, &str_len, sizeof(size_t));
  ...
  read_bytes = read_n(fd, buf, str_len);
...
#define write_func(type)                                        \
  ssize_t write_## type(int fd, type val) {                     \
    return write_n(fd, &val, sizeof(type));   \
  }

#define read_func(type)                                         \
  ssize_t read_## type(int fd, type *val) {                     \
    return read_n(fd, val, sizeof(type));     \
  }
